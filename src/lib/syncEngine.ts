...
async function decryptSyncVault(encrypted: EncryptedVault, masterPassword: string, format: "sync" | "legacy") {
  try {
    ...
    throw new Error("Не удалось расшифровать файл синхронизации. Проверьте, что на обоих устройствах введён одинаковый мастер-пароль. PIN устройств может отличаться. Также файл мог быть повреждён или создан старой несовместимой сборкой.");
  }
}
